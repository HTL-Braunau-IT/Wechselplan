...
- **Native app (Bearer tokens):** with `AUTH_BEARER_ENABLED=true`, `/api` also accepts an Entra access token (`Authorization: Bearer …`) for the iOS/Android app. `src/lib/bearer-auth.ts` verifies the token (signature/issuer/audience, edge-safe via `jose`); `src/lib/request-session.ts` (`getEffectiveSession`) resolves it into the same `Session` shape as the cookie path, reusing `resolveMicrosoftAccess` + local-admin promotion, so **role logic has one source of truth**. The guards (`api-guard.ts`, `require-admin.ts`) call `getEffectiveSession`, so both auth paths flow through every handler unchanged. Middleware verifies the token's authenticity at the edge but defers role enforcement to the handler (the Edge runtime can't reach Graph/Prisma). Off unless the env flag is set.

**Data model** (`prisma/schema.prisma`): Students/Teachers/Classes carry `externalId`/`externalSource`/`isActive`/`lastSyncedAt` for directory sync (soft-deactivate, never hard-delete synced entities). Key relations: a `Schedule` has normalized `ScheduleTurn`/`ScheduleWeek` rows (the legacy `scheduleData` JSON blob is deprecated and null — see `refactor_progress.md` / `docs/MIGRATION_GUIDE.md`). Group membership is **per weekday**: `StudentWeekdayGroup` (student, plan class, year, weekday → group) is the source of truth, read through `src/lib/weekday-groups.ts`; `Student.groupId` / `GroupAssignment` are only the class-wide fallback for classes without per-day rows (see `docs/ARCHITECTURE.md`). Grades live in `Grade`/`FinalGrade`/`NotenEntry`; `SchoolYear` scopes most records by year with a semester change date.
