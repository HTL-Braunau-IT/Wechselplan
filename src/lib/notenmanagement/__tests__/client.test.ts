...
describe('nmRequest', () => {
	...
	it('reuses a stored token belonging to the same user', async () => {
		mockGetStoredToken.mockReturnValue({ token: 'tok', expiresAt: Date.now() + 60_000, username: 'ada' })
		const fetchMock = mockFetchSequence([{ ok: true, body: {} }])
	...
	it('ignores a stored token belonging to a different user', async () => {
		mockGetStoredToken.mockReturnValue({ token: 'tok', expiresAt: Date.now() + 60_000, username: 'grace' })
		const fetchMock = mockFetchSequence([{ ok: true, body: {} }])
	...
	it('clears a rejected token and retries once with the password', async () => {
		mockGetStoredToken.mockReturnValue({ token: 'stale', expiresAt: Date.now() + 60_000, username: 'ada' })
	...
	it('does not retry when the password attempt itself fails', async () => {
		...
		await expect(
			nmRequest('/api/x', {}, { username: 'ada', password: 'wrong' })
	...
	it('surfaces the upstream detail when the retry also fails', async () => {
		...
		mockFetchSequence([
			{ ok: false, body: { error: 'Unauthorized' } },
			{ ok: false, body: { error: 'Transfer failed', details: { error_description: 'LF locked' } } }
		])
	...
	it('prefers an explicitly supplied token over storage', async () => {
		mockGetStoredToken.mockReturnValue({ token: 'stored', expiresAt: Date.now() + 60_000, username: 'ada' })
		const fetchMock = mockFetchSequence([{ ok: true, body: {} }])
