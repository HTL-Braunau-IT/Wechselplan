 ...
 */
function getPeriodTimeRange(scheduleTimes: PDFData['scheduleTimes'], period: 'AM' | 'PM'): string {
 ...
 */
function getBreakTime(breakTimes: PDFData['breakTimes'], period: 'AM' | 'PM' | 'LUNCH'): string {
...
export async function generateSchedulePDF(data: PDFData): Promise<Buffer> {
  ...
  doc.setFontSize(7)
  doc.text(`Letzte Aktualisierung: ${updatedWeekdayName}, ${updatedDate}`, pageWidth / 2, yPos, {
    align: 'center',
  })
  yPos += 6
  ...
  if (data.amAssignments.length > 0) {
    ...
    autoTable(doc, {
      ...
      didParseCell: (data: any) => {
        ...
        if (
          data.section === 'head' &&
          (data.row.index === 1 || data.row.index === 2 || data.row.index === 3)
        ) {
  ...
  doc.text(`erstellt am: ${formatDateGerman(data.updatedAt)}`, pageWidth - margin, footerY, {
    align: 'right',
  })
...
export async function generateNotensammlerPDF(data: NotensammlerPDFData): Promise<Buffer> {
  ...
  const teacherColumnWidth =
    (availableWidth -
      fixedColumnWidth * 2 -
      studentColumnWidth -
      averageColumnWidth * 4 -
      conductWishColumnWidth * 2) /
    (totalTeachers * 2)
...
export async function generateNotensammlerAllClassesPDF(
  data: NotensammlerAllClassesPDFData,
): Promise<Buffer> {
  const doc = new jsPDF({
    orientation: 'landscape',
    unit: 'mm',
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(6)
    doc.setFont('helvetica', 'italic')
    doc.text(`erstellt am: ${formatDateGerman(new Date())}`, pageWidth - margin, pageHeight - 5, {
      align: 'right',
    })
  }
