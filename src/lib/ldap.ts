  ...
  async authenticate(username: string, password: string): Promise<LDAPUser | null> {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        ...
        this.config.bindPassword?.substring(0, 3) || 'undefined',
      )
      console.log(
        'Password value (last 3 chars):',
        this.config.bindPassword?.substring(this.config.bindPassword.length - 3) || 'undefined',
      )
