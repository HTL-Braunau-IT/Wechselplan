    ...
    result[turn.name] = {
      ...
      holidays:
        turn.holidays?.map(h => {
          ...
          const startDate =
            h.holiday.startDate instanceof Date
          ...
          const endDate =
            h.holiday.endDate instanceof Date
 ...
 */
export function parseJsonToNormalized(scheduleData: unknown): ScheduleTurnData[] {
  ...
  for (const [turnName, turnData] of Object.entries(data)) {
    ...
    result.push({
      ...
      holidayIds: Array.isArray(turnData.holidays)
        ? turnData.holidays
            .map(h => (typeof h === 'object' && h !== null && 'id' in h ? Number(h.id) : 0))
            .filter(id => id > 0)
        : [],
    })
  }
...
export function createScheduleTurnData(
  ...
  return {
    ...
    holidays:
      turnData.holidayIds && turnData.holidayIds.length > 0
        ? {
