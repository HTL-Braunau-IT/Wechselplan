...
export function GradeTable(props: GradeTableProps) {
	...
	const renderPeriodLabels = (visible: boolean) => (
		<>
			...
			{visible && showSeparator && (
				<TableHead rowSpan={2} className="w-1 border-l-2 border-muted-foreground/30 p-0" />
			)}
	...
	const renderSummaryHeads = (semester: Semester) => {
		...
		return (
			<>
				<TableHead rowSpan={2} className="w-14 min-w-14 bg-muted p-1 text-center">
				...
				<TableHead rowSpan={2} className="w-14 min-w-14 bg-primary/10 p-1 text-center">
					<span className="whitespace-nowrap text-sm [text-orientation:mixed] [writing-mode:vertical-rl]">
						{endnoteLabel}
					</span>
				</TableHead>
				<TableHead
					rowSpan={2}
					className={cn(
						'w-[105px] min-w-[90px] max-w-[105px] bg-primary/5 p-1 text-center',
						semester === 'first' && 'border-r-4 border-muted-foreground/60'
					)}
				>
					<span className="whitespace-nowrap text-sm [text-orientation:mixed] [writing-mode:vertical-rl]">
			...
			if ('separator' in column) {
				return (
					<TableCell
						...
						className={cn(
							'border-l-2 border-muted-foreground/30 p-0',
	...
	const renderSummaryCells = (student: Student, semester: Semester, missing: boolean) => {
		...
		return (
			<>
				...
				<TableCell
					className={cn('w-14 min-w-14 bg-primary/10 p-1', missing && MISSING_CELL_CLASS)}
				...
				<TableCell
					className={cn(
						'w-[105px] min-w-[90px] max-w-[105px] bg-primary/5 p-1',
						semester === 'first' && 'border-r-4 border-muted-foreground/60',
						missing && MISSING_CELL_CLASS
					)}
				...
				>
					...
					>
						<SelectTrigger className="h-8 w-full min-w-0 max-w-full truncate text-sm">
			...
			<Table className="border-collapse">
				<TableHeader>
					<TableRow>
						<TableHead
							rowSpan={2}
							className="sticky left-0 z-10 w-10 min-w-10 bg-background p-1 text-center"
						>
							<span className="whitespace-nowrap text-sm [text-orientation:mixed] [writing-mode:vertical-rl]">
						...
						<TableHead
							rowSpan={2}
							className="sticky left-10 z-10 w-10 min-w-10 bg-background p-1 text-center"
						>
							<span className="whitespace-nowrap text-sm [text-orientation:mixed] [writing-mode:vertical-rl]">
								{t('notensammler.group', 'Gruppe')}
							</span>
						</TableHead>
						<TableHead rowSpan={2} className="sticky left-[5rem] z-10 w-[200px] bg-background">
				...
				<TableBody>
					{students.map((student, index) => {
						...
						const missing =
							currentTeacherTeachesClass && hasMissingInCurrentSemester(student.id)
						const missingFirst = missing && currentSemester === 'first'
						const missingSecond = missing && currentSemester === 'second'

						return (
							<TableRow key={student.id}>
								<TableCell className="sticky left-0 z-10 w-10 min-w-10 bg-background p-1 text-center font-medium">
									{index + 1}
								</TableCell>
								<TableCell className="sticky left-10 z-10 w-10 min-w-10 bg-background p-1 text-center">
									{student.groupId ?? '-'}
								</TableCell>
								<TableCell className="sticky left-[5rem] z-10 w-[200px] max-w-[200px] bg-background font-medium">
									<StudentPhoto
