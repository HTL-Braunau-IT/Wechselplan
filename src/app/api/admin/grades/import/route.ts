...
export async function POST(request: Request) {
  ...
  try {
    ...
    const classNames = [
      ...new Set(records.map(r => r.className).filter((name): name is string => !!name)),
    ]
    const studentUsernames = [
      ...new Set(
        records
          .map(r => normalizeUsername(r.studentUsername ?? ''))
    ...
    const teacherUsernames = [
      ...new Set(
        records
          .map(r => normalizeUsername(r.teacherUsername ?? ''))
          .filter((u): u is string => !!u),
      ),
    ]
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (
        ...
        errors.push(
          `Row ${rowNum}: Missing required fields (className, studentUsername, teacherUsername, or semester)`,
        )
        continue
      }
      ...
      if (record.grade && record.grade.trim() !== '') {
        ...
        if (!ALLOWED_GRADES.includes(gradeNum)) {
          errors.push(
            `Row ${rowNum}: Grade must be one of: ${ALLOWED_GRADES.join(', ')}, got "${record.grade}"`,
          )
          continue
        }
        grade = gradeNum
    ...
    for (const gradeData of processed) {
      try {
        ...
        errorCount++
        errors.push(
          `Failed to upsert grade: ${error instanceof Error ? error.message : 'Unknown error'}`,
        )
      }
    }
