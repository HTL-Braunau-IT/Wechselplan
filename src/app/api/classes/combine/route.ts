...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (class1Id === class2Id) {
      return NextResponse.json({ error: 'Cannot combine a class with itself' }, { status: 400 })
    }
    ...
    if (!class1) {
      return NextResponse.json({ error: `Class with ID ${class1Id} not found` }, { status: 404 })
    }

    if (!class2) {
      return NextResponse.json({ error: `Class with ID ${class2Id} not found` }, { status: 404 })
    }
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      const allStudents: StudentWithOrigin[] = [
        ...class1StudentsWithOrigin,
        ...class2StudentsWithOrigin,
      ]
    ...
    return NextResponse.json({ error: 'Failed to combine classes' }, { status: 500 })
  }
}
