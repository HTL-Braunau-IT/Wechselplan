...
export async function POST(request: Request) {
  ...
  try {
    ...
    })) as (Omit<Class, 'students'> & { students?: Student[] }) | null
    ...
    const studentsByGroup = classWithStudents.students.reduce<
      Record<number, { name: string; lastName: string }[]>
    >((acc, student) => {
        ...
        if (turnus.weeks) {
          ...
          validDates.forEach((week, dateIndex) => {
            try {
              ...
              console.error(
                `Error writing date at turnus ${turnusIndex}, date ${dateIndex}:`,
                error,
              )
            }
          })
    ...
    if (teacher) {
      ...
      const assignmentsByGroup = teacherRotations.reduce<
        Record<number, { groupName: string; period: string }>
      >((acc, rotation) => {
