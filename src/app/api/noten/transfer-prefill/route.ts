  ...
  try {
    ...
    for (const row of finalGradeRows) {
      ...
      if (row.semester === 'first') {
        storedGrades[row.studentId]!.first = notensammlerEnabled
          ? null
          : normalizeStoredGrade(row.grade)
      } else if (row.semester === 'second') {
        storedGrades[row.studentId]!.second = notensammlerEnabled
          ? null
          : normalizeStoredGrade(row.grade)
      }
    }
      ...
      for (const s of students) {
        ...
        const first =
          dayGradesFirst.length > 0
            ? Math.round((dayGradesFirst.reduce((a, b) => a + b, 0) / dayGradesFirst.length) * 2) /
              2
            : null
        const second =
          dayGradesSecond.length > 0
            ? Math.round(
                (dayGradesSecond.reduce((a, b) => a + b, 0) / dayGradesSecond.length) * 2,
              ) / 2
            : null
    ...
    return NextResponse.json({ error: 'Failed to fetch transfer prefill' }, { status: 500 })
  }
}
