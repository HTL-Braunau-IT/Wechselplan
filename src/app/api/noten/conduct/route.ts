...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < rawUpdates.length; i++) {
      ...
      const studentId =
        typeof u.studentId === 'string'
          ? parseInt(u.studentId, 10)
          : typeof u.studentId === 'number'
            ? u.studentId
            : NaN
      if (isNaN(studentId)) {
      ...
      if (
        u.conductNoteWish !== null &&
        u.conductNoteWish !== undefined &&
        u.conductNoteWish !== ''
      ) {
        if (
          !ALLOWED_CONDUCT_NOTE_WISH.includes(
            u.conductNoteWish as (typeof ALLOWED_CONDUCT_NOTE_WISH)[number],
          )
        ) {
          return NextResponse.json(
            {
