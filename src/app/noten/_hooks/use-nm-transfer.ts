...
export function useNmTransfer({ classId, schoolYearId, selectedGroupId, allGroupIds }: Params) {
			...
			try {
				const results = await Promise.all(
					groupIds.map(async (groupId) => {
						...
						return { groupId, ok: true as const, students: data.students, prefill: data.prefill ?? {} }
					})
				)
	...
	}, [classId, schoolYearId, semester, session?.user?.name, username, password, groups, excluded, grades, close])
