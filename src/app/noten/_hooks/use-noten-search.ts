...
export function useNotenSearch({ schoolYearId, onNavigate }: Params) {
	...
	const gotoNameMatch = useCallback(
		(match: SearchByNameMatch) => {
			...
			setMessage(`${match.lastName} ${match.firstName} – ${match.className}, ${t('noten.gruppe')} ${match.groupId}`)
			onNavigate(match)
		},
		[onNavigate, t]
	)
	...
	const performNameSearch = useCallback(async () => {
		...
		if (matches.length === 0) {
			...
			setMessage(t('noten.searchNoNameResult', { defaultValue: 'Kein passender Schüler gefunden.' }))
			return
		}
