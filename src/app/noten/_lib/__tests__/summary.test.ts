...
import { DEFAULT_WEIGHTS, emptyEntry, type NotenEntryRow, type Student, type TeachingDay } from '../types'
...
describe('computeStudentSummary', () => {
	...
	it('rounds the final grade to a half step', () => {
		...
		expect(computeStudentSummary([student], days, entries, DEFAULT_WEIGHTS)[1]?.calculatedGrade).toBe(1.5)
		...
		// (1 + 2.5) / 2 = 1.75 -> 2
		expect(computeStudentSummary([student], days, uneven, DEFAULT_WEIGHTS)[1]?.calculatedGrade).toBe(2)
	})
