    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed.startsWith('### ')) {
        ...
        if (currentList.length > 0) {
          formatted.push(
            <ul key={`ul-${listKey++}`} className="mb-2 ml-6 list-disc">
              {currentList}
            </ul>,
          )
          currentList = []
      ...
      <DialogContent className="flex max-h-[80vh] max-w-2xl flex-col">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto pr-2">
          {releases.map(releaseItem => (
            <div key={releaseItem.tag_name} className="space-y-3 border-b pb-6 last:border-b-0">
              <div className="flex items-start justify-between gap-4">
                ...
                {releaseItem.html_url && (
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      window.open(releaseItem.html_url, '_blank', 'noopener,noreferrer')
                    }
