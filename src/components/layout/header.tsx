    ...
    <header className="bg-background fixed top-0 right-0 left-0 z-50 border-b">
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          {showMenu && (
            <button
              onClick={toggleMenu}
              className="hover:bg-accent hover:text-accent-foreground rounded-md p-2 focus:outline-none"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="relative">
                    <span className="bg-muted inline-flex h-8 w-8 overflow-hidden rounded-full">
                      <img
                        src="/api/me/photo"
                        alt=""
                        width={32}
                      ...
                      {(!profileLoaded || profileError) && (
                        <span className="text-muted-foreground inline-flex h-full w-full items-center justify-center text-xs font-medium">
                          {profileInitials}
                        </span>
                      )}
                    </span>
      ...
      {showMenu && isMenuOpen && (
        <>
          ...
          />
          <div
            className={`bg-background fixed top-0 left-0 z-50 h-full w-64 transform border-r shadow-lg transition-transform duration-300 ease-in-out ${
